...
/* Provide a concrete implementation of __io_putchar / __io_getchar
 * that uses USART2 (huart2) so that printf/_write and related
 * functions work out-of-the-box. These functions are strong
 * definitions and will override any weak declaration.
 *
 * Note: huart2 is defined in main.c; we declare it extern here.
 */
#include "main.h" /* for UART_HandleTypeDef huart2 declaration */
extern UART_HandleTypeDef huart2;

int __io_putchar(int ch)
{
  uint8_t c = (uint8_t)ch;
  /* Transmit blocking; timeout small to avoid locking if UART not ready */
  if (HAL_UART_Transmit(&huart2, &c, 1, 10) == HAL_OK) {
    return ch;
  }
  return -1;
}
