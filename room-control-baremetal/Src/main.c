...
// === Eventos (guía 10) ===
volatile uint8_t button_event = 0;     // lo setea la ISR de EXTI (con debounce)
volatile char    uart_event_char = 0;  // lo setea la ISR de USART2
...
static uint32_t t_pwm = 0;            // timestamp en ms (compatibilidad)

// ---- Inicialización de periféricos (según tus drivers) ----
static void peripherals_init(void)
{
    rcc_init();

    // GPIOs
    init_gpio(GPIOA, 5, 0x01, 0x00, 0x01, 0x00, 0x00);  // LED PA5 (LD2)
    ...
    // NVIC/EXTI
    nvic_exti_pc13_button_enable();    // botón PC13
    nvic_usart2_irq_enable();          // RX USART2
    NVIC->IP[EXTI15_10_IRQn] = (1U << 4);  // prioridad alta para botón
    NVIC->IP[USART2_IRQn]    = (3U << 4);  // prioridad baja para UART

    // PWM TIM3_CH1 (PA6 en AF2)
...
int main(void)
{
    ...
    // Estado inicial del PWM = 0% (IDLE por defecto)
    pwm_dc = 0;
    tim3_ch1_pwm_set_duty_cycle(pwm_dc);
    t_pwm = ms_counter;

    // ROOM CONTROL (parser y estados del profe)
    room_control_init();
    ...
    while (1) {
        ...
        if (button_event) {
            ...
            // Forzar LED PA5 encendido 3 s (según lógica del profe)
            led_forced_on = 1;
            led_on_time   = ms_counter;
            set_gpio(GPIOA, 5);

            // Notificar a la app (máquina de estados)
        ...
        // (Si la IRQ RX funciona bien, no hace falta polling aquí)
    }
}
